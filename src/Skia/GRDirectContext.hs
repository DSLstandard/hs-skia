  ...
  allocateSKObjectNeverNull'
    ( evalManaged do
        case options of
          ...
          Just options -> do
            options' <- storable (marshalContextOptions options)
...
'Acquire' releases.
-}
createGl :: (MonadResource m) => GRGlInterface -> Maybe ContextOptions -> m (ReleaseKey, GRDirectContext)
createGl iface opts =
  privCreateHelper
...
'Acquire' releases.
-}
createVulkan :: (MonadResource m) => Gr_vk_backendcontext -> Maybe ContextOptions -> m (ReleaseKey, GRDirectContext)
createVulkan vkbackend opts =
  privCreateHelper
    (Managed.with (storable vkbackend) \vkbackend -> gr_direct_context_make_vulkan vkbackend)
    (\opts -> Managed.with (storable vkbackend) \vkbackend -> gr_direct_context_make_vulkan_with_options vkbackend opts)
    opts
...
'Acquire' releases.
-}
createDirect3D :: (MonadResource m) => Gr_d3d_backendcontext -> Maybe ContextOptions -> m (ReleaseKey, GRDirectContext)
createDirect3D d3dbackend opts = do
  privCreateHelper
    (Managed.with (storable d3dbackend) \d3dbackend -> gr_direct_context_make_direct3d d3dbackend)
    (\opts -> Managed.with (storable d3dbackend) \d3dbackend -> gr_direct_context_make_direct3d_with_options d3dbackend opts)
    opts
...
getResourceCacheUsage ctx = evalManaged do
  resourceCount' <- managed alloca
  maxResourcesBytes' <- managed alloca
