...
-- Must explicitly import, or else template-haskell wouldn't work.
...
-- * Google Skia's `modules/skunicode`

--
-- This Skia module is almost a one-header implementation in Google Skia, so we
-- will not bother creating a module named Skia.Unicode.Types and instead of put
-- everything here.
